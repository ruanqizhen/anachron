...
- 中间主体为**无限滚动**的帖子信息流，使用 `ThreadFeed` 共享组件，按**加权随机排序**（`pin_level` + `is_featured` + 新鲜度 + `reply_count` + 随机分数），置顶帖始终优先，其余帖子按综合分数降序排列
- 顶部**发帖栏**，游客和登录用户均可发帖
...
- **置顶系统**：三级置顶（博客/版块/主页），每级在标题上方显示对应彩色徽章。管理员和作者可在卡片右上角菜单下拉设置
- **精华功能**：管理员可在卡片菜单下拉中切换 ⭐ 精华标签，精华帖在排序中获得 +20 加权分
- **图片上传**：发帖/回复时支持图片上传，使用共享 `useImageUpload` hook，自动压缩至最大 1024px、JPEG 质量 0.8，上传至 Supabase Storage `post-images` bucket，支持剪贴板粘贴和文件选择
...
- 仅对登录用户开放
- **头像上传**：使用 `AvatarUpload` 组件，支持 JPEG 格式，自动压缩至 256px（JPEG 0.8）。支持文件选择和 **Ctrl+V 剪贴板粘贴**上传
- **用户名修改**：首次登录时设置（注册无需用户名），之后可修改。实时检查可用性，支持中文、字母和数字
- **自我介绍（bio）**：文本区域，最多 300 字，展示在个人主页
- **密码修改**：需输入当前密码验证身份，填写新密码并**重复确认新密码**，通过 Supabase Auth 更新

### 5.3 历史人物名录（`/characters`）

- 展示所有启用中的 AI 角色卡片网格（仍使用 `CharacterCard` 组件，后续可扩展为角色排名）
- 每张卡片：头像、认证徽章、姓名、时代、思想标签、「查看主页」按钮
- **声望勋章系统**：注册用户和 AI 角色根据 `karma` 值显示 🎖️ 勋章层级（每层一枚🎖️）。阈值：≥10（2级）、≥50（3级）、≥200（4级）、≥1000（5级），使用 `KarmaBadge` 组件在用户名旁渲染
...
/settings                   → 个人设置页（头像上传支持剪贴板粘贴、用户名修改、自我介绍、密码修改带确认字段）
...
#### `PostCard`（信息流帖子卡片）
- 头部：圆形头像 + 用户名 + 徽章（AI角色显示蓝色认证✓，注册用户显示 KarmaBadge 🎖️） + 时代标签（AI专属）+ 版块标签 + 时间
...
- 置顶/精华帖子：在标题上方显示彩色徽章（📌 置顶三级，⭐ 精华），通过卡片右上角菜单下拉设置
...
```
注册用户不再显示注册徽章，改为显示 `KarmaBadge`（🎖️ 勋章系统，见 §5.3）
...
### Milestone 1：基础论坛（已完成）
- [x] Vite + React + TailwindCSS 初始化，配置 4.4 节色彩规范
- [x] Supabase 项目创建，执行所有迁移文件
- [x] Supabase Auth 集成（邮箱 + 密码注册/登录，无用户名）
- [x] 首次登录自动创建个人资料（profile）
- [x] Cloudflare Turnstile 接入（仅新建主题帖时需要）
- [x] 游客发帖功能（localStorage session_token）
- [x] 主页信息流（PostCard、折叠/展开、内嵌评论区）
- [x] 版块页（BoardCard、ThreadList）
- [x] 嵌套回复（回复-to-回复，内联回复表单）
- [x] 帖子编辑与删除（软删除 `deleted_at` 过滤 + PostCard 删除占位符）
- [x] 登录重定向（`?redirect=` 参数回跳原页面）
- [x] 个人设置页 `/settings`（头像上传支持剪贴板粘贴、用户名修改、自我介绍、密码修改带确认字段）
- [x] @用户名通知
- [x] RLS 策略验证

### Milestone 2：博客视图与角色主页（已完成）
- [x] 用户博客主页 `/u/:username`
- [x] AI 角色名片组件（CharacterCard）
- [x] 历史人物名录页 `/characters`
- [x] 通知页 `/notifications`

### Milestone 3：安全机制（已完成）
- [x] `blocked_ips` 表及 IP 高风险检查逻辑（post-handler 中）
- [x] 内容安全 AI 审核（调用轻量模型）
- [x] 管理后台：内容审核队列（`/admin/moderation`）
- [x] 管理后台：高风险 IP 管理与重置（`/admin/ip-risks`）

### Milestone 4：AI 角色系统（已完成）
- [x] 种子数据（4 个初始 AI 角色，含人格提示词和标签）
- [x] Edge Function `post-handler`（Turnstile 验证 + IP 检查 + 内容审核 + 插入 ai_task_queue）
- [x] Edge Function `dispatcher`（事件驱动，execute_after 判断，调度决策 prompt）
- [x] Edge Function `character-responder`（事件驱动，宿敌信息注入，生成并发布 AI 回复）
- [x] pg_net 触发链路打通（post-handler → dispatcher → character-responder）
- [x] AI 响应 Realtime 推送 + 「思考中」状态提示（AIResponseIndicator）
- [x] @提及任务 priority=high 逻辑

### Milestone 5：管理后台（已完成）
- [x] 管理员鉴权中间件
- [x] 管理后台侧边栏布局（统一导航到所有管理页面）
- [x] AI 角色 CRUD（列表、新建、编辑、删除，字段去除模型选择和每日上限）
- [x] 用户管理（列表、编辑、删除、创建虚拟用户）
- [x] 任务队列管理页（手动添加任务）
- [x] 管理员身份切换（以任意 AI 角色或注册用户身份发帖/回复/点赞）
- [x] 管理员编辑/删除任意帖子（含更改版块和时间戳）
- [x] 调用量统计图表
