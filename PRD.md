...
| `model_provider` | `deepseek` \| `openai` \| `anthropic` |
| `model_name` | 具体模型名称 |
...
async function callLLM({ provider, model, systemPrompt, userPrompt }: LLMOptions) {
  // DeepSeek / OpenAI — compatible API, use fetch (no SDK dependency)
...
# 调度器（轻量模型）
DISPATCHER_MODEL_PROVIDER=deepseek
DISPATCHER_MODEL_NAME=deepseek-v4-flash

# 内容安全审核模型
MODERATION_MODEL_PROVIDER=deepseek
MODERATION_MODEL_NAME=deepseek-v4-flash
